...
class Interaction():
    ...
    def getRevChains(self):
        return str(self.chain2) + "/" + str(self.chain1)
    ...
    def getTotal(self):
        return "(" + str(self.pos1) + ") " + self.base1 + " - " +self.int+" - " + self.base2 + " (" + str(self.pos2) + ") "

    def getRedTotal(self):
        return "(" + str(self.pos2) + ") " + self.base2 + " - " +self.int[0:1]+self.int[2:3]+self.int[1:2]+" - " + self.base1 + " (" + str(self.pos1) + ") "
...
def getName(name):
    if(len(name) < 27): #magic nums
        return name[0:4]
    return name[0:4]+ "-"+ name[-20:-13] #whats going on here?
...
def divideLine(line):
    ...
    for i in range(len(line) - 1):
        if (line[i:i + 1] == ","):
            pos.append(i)
    #return [line[0:pos[0]],line[pos[0]+1:pos[1]],line[pos[1]+1:len(line)-1]]
    return [line[0:pos[0]-1],line[pos[0]+2:pos[1]-1],line[pos[1]+2:len(line)-2]]
...
def simplifyInt(input_str):
    # Step 1: Remove 'n' and 'a'
    result = input_str.replace('n', '').replace('a', '')

    # Step 2: Replace 'B' with 'W'
    ...
    # Step 3: Replace 'sS' with 'RS'
    result = result.replace('Ss', 'RS')

    # Step 4: Replace 'Ss' with 'SR'
    result = result.replace('sS', 'SR')
    return result
def getReverse(str):
...
def removeRedundancy(modules):
    ...
    for a in range(len(modules)):
        ...
        for b in range(len(double)):
            if(modules[a][0].getPos1() == double[b][0].getPos2()):
                if (modules[a][1].getPos1() == double[b][1].getPos2()):
                    isRed = False
            if (modules[a][1].getPos1() == double[b][0].getPos2()):
                if (modules[a][0].getPos1() == double[b][
                    1].getPos2()):
                    isRed = False
        if(isRed):
...
def removeMRedundancy(modules):
    matrix = []
    double = []
    for a in range(len(modules)):
        isRed = True
        for b in range(len(double)):
            if (modules[a][0].getChain1() == double[b][0].getChain1()):
                if(modules[a][0].getPos1() == double[b][0].getPos1()):
                    if (modules[a][0].getPos2() == double[b][0].getPos2()):
                        if (modules[a][1].getPos1() == double[b][1].getPos1()):
                            if (modules[a][1].getPos2() == double[b][1].getPos2()):
                                isRed = False
        if(isRed):
            matrix.append(modules[a])
            double.append(modules[a])
...
def findModuleAB(modules):
    ...
    return removeMRedundancy(matrix)
...
def removeDups(mods):
    for a in range(len(mods) - 1, -1, -1):
        ...
        for b in range(len(mods)):
            if (mods[a][0].getTotal() == mods[b][0].getTotal()):
                if (mods[a][1].getTotal() == mods[b][1].getTotal()):
                    if (mods[a][0].getChains() != mods[b][0].getChains()):
                        if (mods[a][1].getChains() != mods[b][1].getChains()):
                            red = True
        if (red):
            del mods[a]
    return mods
...
for file in range(1,len(sys.argv)-1,3):
    name = os.path.basename(sys.argv[file])
    pdb = name[0:4]

    file_name = "Modules_fr3d_" + str(pdb)+"_"+ date +".xlsx"
    detail = sys.argv[file]
    stack = sys.argv[file+1]
    #sugarrib = sys.argv[file+2]

    #creates the array of all interactions
    basepairs = []
    #detail file
    with open(detail,'r') as tfile:
        for line in tfile:
            basepairs.append(createInteraction(line))
    #sugar ribose file
    #with open(sugarrib, 'r') as rfile:
     #   for line in rfile:
      #      basepairs.append(createInteraction(line))
    ...
    moduleAB = removeDups(moduleAB)
    moduleA = removeDups(moduleA)
    moduleB = removeDups(moduleB)
    moduleBprime = removeDups(moduleBprime)
    moduleC = removeDups(moduleC)
    moduleD = removeDups(moduleD)
    ...
    print(len(moduleAB)+len(moduleC)+len(moduleD))

    writeSheet(moduleAB, abAbun, "Output A+B+Bprime", str(pdb))
    makeTable(moduleAB, abAbun, "Tables Output A+B+Bprime")
    writeSheet(moduleA, aAbun, "OutputA", str(pdb))
    makeTable(moduleA, aAbun, "Tables Output A")
    writeSheet(moduleB, bAbun, "OutputB", str(pdb))
    makeTable(moduleB, bAbun, "Tables Output B")
    writeSheet(moduleBprime, bpAbun, "Output B-prime", str(pdb))
    makeTable(moduleBprime, bpAbun, "Tables Output B-prime")
    writeSheet(moduleC, cAbun, "OutputC", str(pdb))
    makeTable(moduleC, cAbun, "Tables Output C")
    writeSheet(moduleD, dAbun, "OutputD", str(pdb))
