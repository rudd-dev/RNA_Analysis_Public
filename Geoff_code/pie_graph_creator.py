...
#creates graph and uses the variables: sheet data, type of structure, sheet name, array of colors for each int
def writeGraph(out,struct,name,colors):
    # Create a 17x17 grid of subplots for the first figure
    # This includes one row/column for labels and 16x16 data cells
    fig, axs = plt.subplots(17,17, figsize=(6, 6))

    #Number of pie-chart categories (based on number of colors)
    p = len(colors)

    # Maximum value of the total-count column (used later for scaling)
    max = out.iloc[:, p+1].max()

    # Stores the data for all the values in the last column(Total)
    total = out.iloc[:, p+1].sum()

    # Index to iterate through rows of the dataframe
    index = 0
    ...
    # Loops through the 16x16 data grid (excluding label row/column)
    for a in range(1,17):
        for b in range(1,17):
            # Extract the current row of data
            data = out.iloc[index]

            # If the total count for this cell is zero,
            # draw a blank (white) pie chart
            if (data.iat[p+1] == 0):
                axs[b,a].pie([1, 0], colors=['#FFFFFF', '#FFFFFF'])
            else:
                # Otherwise, draws a pie chart using extracted category data
                axs[b,a].pie(extractData(data,p),colors=colors,radius=1.5,startangle = 90,counterclock=False)
            # Move to the next dataframe row
            index += 1

    # Creates a legend using the colors and column names
    ...
    # Adds total count of all modules included in data to the top-left corner
    axs[0, 0].text(0, .5, 'n='+str(total), ha='center', va='center', fontsize=6)
    axs[0, 0].axis('off')

    # Adds the header labels along the top row and left column(ex C-G)
    for i in range(1,17):
    ...
    # Save the first figure (uniform-sized pies)
    plt.savefig(struct+name+'.svg', format='svg', bbox_inches='tight')

    # -------- SECOND FIGURE (size-scaled pie charts) -------- #

    # Create a new figure for scaled pies
    ...
    for a in range(1,17):
        for b in range(1,17):
            ...
            if (data.iat[p+1] == 0):
                ...
                # Scales pie radius based on percentage of total modules identified
    ...
    # Saves the size-scaled figure
    plt.savefig(struct+name+'-Sized.svg', format='svg', bbox_inches='tight')
    # Close the figure to free memory and save it
    plt.close()

#file_path refers to the title of the file that will be analyzed to make the graphs. Needs to be in the same folder as this python script file
file_path = "Modules_fr3d_9QF4_3_7_2026.xlsx"#'Stack_Abundance_Ribozyme.xlsx'
#used to add name of structure to before the file name
structure = "Ribozyme_"
...
#utE = pd.read_excel(file_path, sheet_name='Output SS')
#utAE = pd.read_excel(file_path, sheet_name='Output 2H + SS')
...
#Creates the graph using the writeGraph method
#Name of output file combines the variables "structure" and String in writeGraph

writeGraph(outA,structure,"OutputA_test_9QF4",['#0000ff','#ff0000'])
